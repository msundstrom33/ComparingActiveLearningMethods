...
aggregate_transition_network <- function(Instructor) {
  ...
  color_palette <- colorRampPalette(c("white", "tan"))
...
cluster_colors<-c("#85b84b","#e17f35","#ec54a1","#837ebb","#1b9e77")
...
for (i in all_classes) {
 ...
 #  outstrengths<-as.data.frame(strength(net, mode="out"))
  indegrees<-as.data.frame(igraph::degree(net, mode="in"))
  outdegrees<-as.data.frame(igraph::degree(net, mode="out"))
...
centralities<-ggplot(data=network_characteristics_means,aes(x=Code,y=Value,color=as.factor(Cluster)))+geom_point(size=2.7)+geom_line(aes(group=Cluster),linetype="solid",size=0.2)+facet_wrap(~Centrality,ncol=1,scales="free_y",strip.position = "right")+theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.ticks.y = element_line(color = "black", size = 0.5),axis.ticks.x=element_line(color = "black", size = 0.5),axis.title = element_text(size=10,color='black'),axis.text = element_text(size=9,color='black'),axis.text.x = element_text(size=9,color='black',angle=0)) + theme(strip.background = element_rect(colour='black', size=0.5),strip.text=element_text(size=9,color='black'))+theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5)) + theme(legend.position = "bottom",legend.title=element_blank(),legend.text=element_text(size=9,color='black'),legend.margin = margin(t = -10))+scale_color_manual(values=cluster_colors<-c("#1b9e77","#85b84b","#e17f35","#ec54a1","#837ebb"), labels=c("Clicker lecture","Dialogic clicker lecture","Dialogic lecture with short groupwork activities","Short groupwork activities","Long groupwork activities"))+ylab("Average value")+xlab("COPUS code")+guides(color = guide_legend(nrow = 5))+ theme(legend.key.spacing.y = unit(-7, "pt"))

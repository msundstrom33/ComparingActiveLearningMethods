...
aggregate_transition_network <- function(Instructor) {
  ...
  color_palette <- colorRampPalette(c("white", "grey60"))
...
attributes<-attributes[is.na(attributes$Implementation)==FALSE,] # Remove one course that did not collect video data
...
compare<-ggplot() +
  ...
  scale_fill_gradient2(low = "white", high = "#0072b2",limits = c(0,1),name="Jaccard \nindex", breaks=c(0,0.5,1)) +
  ggnewscale::new_scale_fill() +geom_tile(data = subset(df, triangle == "upper"),
                                          aes(x = Col, y = Row, fill = Value)) +
  scale_fill_gradient(low = "white", high = "#009e74",limits = c(0,1),name="Cosine \nsimilarity", breaks=c(0,0.5,1)) +
  scale_y_reverse() +  
  labs(x = "Instructor", y = "Instructor") +
  theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.ticks=element_blank(),axis.title = element_text(size=10,color='black'),axis.text = element_blank(),axis.title.x = element_text(margin = margin(t = -5)),axis.title.y = element_text(margin = margin(r = -8)),legend.text = element_text(size = 7),legend.title=element_text(size = 7),legend.margin = margin(0, 2, 0, -10))+theme(legend.key.height = unit(0.4, "cm"),legend.key.width = unit(0.4, "cm"))
...
cluster_colors<-c("#56b3e9","#cc79a7","#e69d00","#d55e00","#0072b2")
...
centralities<-ggplot(data=network_characteristics_means,aes(x=Code,y=Value,color=as.factor(Cluster)))+geom_point(size=2.5)+geom_line(aes(group=Cluster))+facet_wrap(~Centrality,ncol=1,scales="free_y",strip.position = "right")+theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.ticks.y = element_line(color = "black", size = 0.5),axis.ticks.x=element_line(color = "black", size = 0.5),axis.title = element_text(size=10,color='black'),axis.text = element_text(size=9,color='black'),axis.text.x = element_text(size=9,color='black',angle=0)) + theme(strip.background = element_rect(colour='black', size=0.5),strip.text=element_text(size=9,color='black'))+theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5)) + theme(legend.position = "bottom",legend.title=element_blank(),legend.text=element_text(size=9,color='black'),legend.margin = margin(t = -10))+scale_color_manual(values=cluster_colors<-c("#0072b2","#56b3e9","#cc79a7","#e69d00","#d55e00"), labels=c("Clicker lecture","Dialogic clicker lecture","Dialogic lecture with short groupwork activities","Short groupwork activities","Long groupwork activities"))+ylab("Average value")+xlab("COPUS code")+guides(color = guide_legend(nrow = 5))+ theme(legend.key.spacing.y = unit(-7, "pt"))

centralities

ggsave(centralities,filename = "ClusterCentralities.pdf",width=5,height=4.5,units="in")
...
pdf("Cluster1_Example.pdf", width=6, height=6)  
aggregate_transition_network("PeerInstruction_Course3")
dev.off()

pdf("Cluster2_Example.pdf", width=6, height=6)  
aggregate_transition_network("ISLE_Course3")
dev.off()

pdf("Cluster3_Example.pdf", width=6, height=6)  
aggregate_transition_network("Tutorials_Course2")
dev.off()

pdf("Cluster4_Example.pdf", width=6, height=6) 
aggregate_transition_network("SCALEUP_Course3")
dev.off()

pdf("Cluster5_Example.pdf", width=6, height=6)  
...
x<-as.table(Z)
names(x)<-c("LPA profile","Class size (<45 or >45)","Active learning method","Discipline (physics\nor astronomy)","Public or private","Research designation\n(R1/R2 or other)","PhD-granting")
df <- data.frame(
...
seg<-ggplot(df, aes(x = Variable, y = Zscore)) +
  ...
  ylim(-2.5, 7)+theme_minimal() + theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),axis.ticks.y = element_line(color = "black", size = 0.5),axis.ticks.x=element_line(color = "black", size = 0.5),axis.title = element_text(size=10,color='black'),axis.text = element_text(size=9,color='black'),axis.text.x = element_text(size=9,color='black',angle=45,hjust=1,vjust=1)) + theme(strip.background = element_rect(colour='black', size=1),strip.text=element_text(size=9,color='black'))+theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5)) + theme(legend.position = "bottom",legend.title=element_blank(),legend.text=element_text(size=9,color='black'))
...
seg_lpa<-ggplot(df_fraction, aes(x = Cluster, y = Fraction, fill = LPA_Profile)) +
  ...
  guides(fill = guide_legend(nrow = 4, byrow = TRUE))+scale_fill_manual(values=c("#525252","#737373","#969696","#bdbdbd"),name = "LPA profile")+guides(fill = guide_legend(keywidth = 0.8, keyheight = 0.8))
...
ggarrange(grid.arrange(seg_lpa, seg_class, ncol = 1,heights=c(1,1.9))) %>%
  ggexport(filename = "Segregation_Distributions.pdf",width=5,height=3.7,units="in")
...
colnames(hedgesg)<-c("Method","Instructor","NMatched","Percent.Matched","HedgesG","LowerCI","UpperCI","Variance")

membership_learning<-cbind(hedgesg,attributes$Cluster)

colnames(membership_learning)<-c("Method","Instructor","NMatched","Percent.Matched","HedgesG","LowerCI","UpperCI","Variance","Cluster")
...
pooled_plot<-cbind(pooled_plot,c("Dialogic clicker\nlecture (n = 382)","Dialogic lecture with\nshort groupwork\nactivities (n = 140)","Short groupwork\nactivities (n = 179)","Long groupwork\nactivities (n = 135)","Clicker lecture\n(n = 810)"))
colnames(pooled_plot)<-c("g","g_lower","g_upper","zvalue","pvalue","method")
pooled_plot$method<-factor(pooled_plot$method,levels=c("Clicker lecture\n(n = 810)","Dialogic clicker\nlecture (n = 382)","Dialogic lecture with\nshort groupwork\nactivities (n = 140)","Short groupwork\nactivities (n = 179)","Long groupwork\nactivities (n = 135)"))



learn<-ggplot() +
  geom_hline(yintercept=0,color = "black")+
  geom_pointrange(data=pooled_plot,aes(x=method, y=g,ymin = g_lower, ymax = g_upper,colour=method),size=0.8)+
  ...
  theme(panel.border = element_rect(color = "black", fill = NA, size = 0.5)) + theme(legend.position = "none",legend.title=element_blank(),legend.text=element_text(size=8,color='black'),axis.text.x = element_text(size=9,color='black',angle=45,hjust=1,vjust=1))+ylab("Effect size (Hedges' g)")+scale_colour_manual(values=c("#0072b2","#56b3e9","#cc79a7","#e69d00","#d55e00"))+xlab("Instruction type")


learn

ggsave(learn,filename = "Cluster_EffectSizes.pdf",width=4,height=3.25,units="in")
